...
for key, text in words.items():
    try:
        ...
        tts.save(f'public/audio/{key}.mp3')
        print(f'✓ {key}.mp3')
    except Exception as e:
        print(f'✗ Error: {key} - {e}')

# Generate number audio files
for num, text in numbers.items():
    try:
        tts = gTTS(text=text, lang='es', slow=False)
        tts.save(f'public/audio/{num}.mp3')
        print(f'✓ {num}.mp3')
    except Exception as e:
        print(f'✗ Error: {num} - {e}')

print(f'\n✅ Generated {len(words) + len(numbers)} audio files!')
print('Total size: ~2-3MB')